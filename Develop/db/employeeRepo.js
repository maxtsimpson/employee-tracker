...
class employeeRepositry {
    ...
    async getEmployeeByEmployeeString(employeeString) {
        ...
        return await this.db.query(this.getEmployeeByNameAndTitleQuery(), ["role", first_name, last_name, title])
            .then((result) => {
                console.log({result})
                Promise.all([this.createEmployeeObject(result[0])]).then((values) => {
                    console.log(values[0])
                    return values[0]
                })
                // return await result.map(async (e) => await this.createEmployeeObject(e))
                // new Promise((resolve, reject) => {
                //     resolve(result.map(async (e) => await this.createEmployeeObject(e)))
                // })
            })
            // .then(result => Promise.all([result.map(e => this.createEmployeeObject(e))[0]]))        
            // .then(result => Promise.all([result.map(e => this.createEmployeeObject(e))[0]]))    
            .catch((error) => { throw error })
    }
